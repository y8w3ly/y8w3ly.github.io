...
![rbg](/assets/img/Alpacahack/rbg.png)
...
Because the LCG wraps mod N, consecutive ciphertexts are related by a factor that depends on a small carry `(k ∈ {0,1,2})`.
...
That’s where the whole thing breaks open.
...
We have:
+ `X ≡ m^{1337} (mod N)`
+ `Z ≡ m^{-N} (mod N)` so `Z^{-1} ≡ m^{N} (mod N)`
and we have `1337` and `N` are coprime. So according to bezout there exist `u` and `v` such that `1337·u + N·v = 1`.
...
+ `m^{N} ≡ Z^{-1}` (just compute `inverse(Z,n))
Then finally : 
```python
...
So a small recap : 
The LCG’s reduction by `N` injects a small unknown integer (`k_i ∈ {0,1,2}`) into the exponent recurrence. That small set produces at most three multiplicative possibilities between successive ciphertexts. By factoring out the `c_i^3` term, we isolate `X = m^{1337}` multiplied by `Z^{k_i}` and thus detect `X` and `Z`. Once `X` and `Z` are known, simple exponent arithmetic via bezout gives `m`.
